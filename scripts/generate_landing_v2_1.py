#!/usr/bin/env python3
"""LANDING_PAGE V2.1 BALANCE FIX GENERATION
   Pipeline: VISUAL_PRODUCTION_PIPELINE_V3 STEP 2
   Engine: Jimeng (即梦) via Seedream Ark
   Input: STRUCTURE_SPEC_LANDING_V2_1
   Fix: Restore WeChat login button + functional UI while preserving world entry feel
   Output: apps/miniapp/static/scene/landing_v2_1.jpg
"""
...
OUTPUT = ROOT / "apps/miniapp/static/scene/landing_v2_1.jpg"
SPEC = ROOT / "assets/visual-pipeline/landing_v1/landing_v2_1_generation_spec.json"
...
PROMPT_V2_1 = (
    "AIGU VALLEY landing page V2.1 — BALANCE FIX. "
    "Full mobile screen 9:16. "
    "This is a WORLD ENTRY page but also a FUNCTIONAL PRODUCT ENTRY. "
    "Portal gate occupies 65-70% of canvas — strong dominance but leaves room for bottom UI. "
    "Pure void background — deepest black (#050505) fading to indigo void (#0F0530). "
    "Zero background objects. Zero textures. Background is VOID canvas. "
    "Center: WORLD ENTRY GATE — massive transdimensional portal, "
    ...
    "Portal rim has minimal resonance particles — gold/violet dust only near portal edge. "
    "Atmosphere: sacred, world-descending, awe — NOT sci-fi, NOT combat. "
    "Top center: ritual light script 'AR游伴' — thin calligraphic, divine glow, minimal signature. "
    "Bottom zone MUST contain: primary action button with text '谒见世界 · 微信登录进入' — "
    "this button MUST be recognizable as a WeChat login entry action with a WeChat icon, "
    "glowing energy button style, prominent and actionable. "
    "Bottom left: hybrid text '故事 · 往世书'. "
    "Bottom right: hybrid text '说明 · 启明录'. "
    "All bottom UI must be readable as FUNCTIONAL ENTRY POINTS not purely decorative text. "
    "UI density is functional-symbolic hybrid — 12-15% of canvas. "
    "The user is both WITNESSING a world and ABOUT TO ENTER it via WeChat login. "
    "CRITICAL: WeChat login button must be VISIBLE and RECOGNIZABLE as a login action. "
    "This is NOT optional. "
    "VISUAL PRIORITY CONTROL RULES: "
    "[CENTER_ANCHOR] Portal 65-70% dominance — strong but not absolute. "
    "[UI] Bottom UI zone with WeChat login is functional and visible, 12-15% of canvas. "
    "[BACKGROUND] Pure void. Zero competition. "
    "[Z_ORDER] Void → particles → Portal (65-70%) → Title → Bottom UI including WeChat login button."
)
...
print("=" * 60)
print("LANDING PAGE V2.1 — BALANCE FIX GENERATION")
print("=" * 60)
...
backup = CANDIDATES / f"landing_v2_1_balance_{int(time.time())}.jpg"
