#!/usr/bin/env python3
"""CH06 Placeholder Audit — read-only."""
...
REPORT = ROOT / "docs/audit/CH06_PLACEHOLDER_AUDIT_REPORT.md"
...
def main() -> int:
    ...
    ph_audit = autopilot.validate_placeholder(CH06)
    ...
    if ch["id"] == "ch06_field_completion":
        findings["pass"].append("chapter id ch06_field_completion")
    else:
        findings["blockers"].append(f"id mismatch: {ch['id']}")
    if ch["title"] == "归位觉醒":
        findings["pass"].append("title 归位觉醒")
    else:
        findings["blockers"].append(f"title mismatch: {ch['title']}")
    if ch.get("imprint_album", {}).get("album_code") == "F":
        findings["pass"].append("album_code F")
    else:
        findings["blockers"].append("album_code not F")
    if ch.get("previous_chapter") == "ch05_field_return":
        findings["pass"].append("previous_chapter ch05_field_return")
    else:
        findings["blockers"].append("previous_chapter mismatch")

    comp = ch.get("completion", {})
    if comp.get("completion_mark") == "觉醒印记":
        findings["pass"].append("completion_mark 觉醒印记")
    else:
        findings["blockers"].append("completion_mark mismatch")
    if comp.get("memorial_title") == "觉醒见证者":
        findings["pass"].append("memorial_title 觉醒见证者")
    else:
    ...
    if CANON_PATH.exists() and CANON_PATH.read_text(encoding="utf-8").strip():
        findings["pass"].append("CH06_CONTENT_CANON_V1.md present")
    else:
        findings["warnings"].append("docs/content/canon/CH06_CONTENT_CANON_V1.md missing or empty")

    if story.get("source_ref") == "docs/content/canon/CH06_CONTENT_CANON_V1.md":
    ...
    if dc_ar and not (ROOT / "docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH06.md").exists():
    ...
    ch05_story = autopilot.read_json(ROOT / ch05["story"])
    if ch05_story["chapters"][0].get("next_chapter") != CH06["id"]:
        findings["warnings"].append(
            f"CH05 next_chapter={ch05_story['chapters'][0].get('next_chapter')} != {CH06['id']}"
        )
    ...
    lines = [
        ...
        "",
        f"**Mission:** 44 · CH06_PLACEHOLDER_AUDIT  ",
        f"**Generated:** {ts}  ",
        f"**Scope:** CH06 L2 placeholder factory · `data/story|relics|rights|ar/ch06_*`  ",
        f"**Upstream:** [`CH06_CONTENT_CANON_V1.md`](../content/canon/CH06_CONTENT_CANON_V1.md)  ",
        "",
        "## Verdict",
        "",
        f"## **`CH06_PLACEHOLDER_AUDIT = {'PASS' if ready else 'FAIL'}`**",
        "",
        f"**`CH06_PLACEHOLDER_READY = {'YES' if ready else 'NO'}`**",
        "",
        "---",
        "",
        "## 1. 审计方法",
    ...
    print(
        json.dumps(
            {"ready": ready, "blockers": blockers, "warnings": warnings, "report": str(REPORT.relative_to(ROOT))},
            ensure_ascii=False,
            indent=2,
        )
    )
