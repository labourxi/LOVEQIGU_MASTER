...
REPORT = ROOT / "docs/content/CH09_CONTENT_AUDIT_CREATE_REPORT.md"
CANON_PATH = ROOT / "docs/content/canon/CH09_CONTENT_CANON_V1.md"
DC_REGISTRY = ROOT / "docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH09.md"
...
CH09 = next(c for c in autopilot.load_registry() if c["code"] == "CH09")


def main() -> int:
    paths = autopilot.chapter_paths(CH09)
    audit = autopilot.validate_chapter(CH09, check_dc=True)
    ...
    if ch.get("imprint_album", {}).get("album_code") == "H":
        ok("album H")
    else:
        fail("album_code")
    ...
    if comp.get("completion_mark_relic_ref") == "relic_ch09_field_future_seal":
        ok("completion ref")
    else:
        fail("completion_mark_relic_ref")

    if comp.get("completion_mark") == "未来印记":
        ok("completion_mark")
    else:
        fail("completion_mark mismatch")

    if ch.get("previous_chapter") == "ch08_field_echo_legacy":
        ok("previous_chapter")
    else:
    ...
    for w in audit["warn"]:
        if "link:" in w:
            warn("W-001", "Low", w.replace("link: ", "CH08→CH09: "))
    ...
    seal = next(r for r in relics if r["id"] == "relic_ch09_field_future_seal")
    if (
    ...
    n4_ar = next(e for e in events if e["node_id"] == "n4_social_impact")
    ...
    if dc_events == ["ar_ch09_completion_v1"]:
        ok("DC only completion AR")
    else:
        fail(f"DC on wrong events: {dc_events}")

    if DC_REGISTRY.exists() and "dc_ch09_future_poster" in DC_REGISTRY.read_text(encoding="utf-8"):
        ok("DC registry")
    else:
        fail("DC registry missing dc_ch09_future_poster")

    if ch.get("next_chapter") == "TBD":
        warn("W-004", "Info", "CH09 next_chapter: TBD — CH10+ Canon 未述 · 符合暂停边界")
    ...
    lines = [
        ...
        "|------|:----------:|:------:|:------------:|:------:|",
        f"| `data/story/ch09_chapters.json` | PASS | `loveqigu.story.chapters.v1` | chapter shell + {counts['nodes']} nodes | **PASS** |",
        f"| `data/relics/ch09_relics.json` | PASS | `loveqigu.relics.v1` | {counts['relics']} relics + asset_boundary | **PASS** |",
        f"| `data/rights/ch09_rights.json` | PASS | `loveqigu.rights.v1` | {counts['rights']} rights + relic_refs_all | **PASS** |",
        f"| `data/ar/ch09_ar-events.json` | PASS | `loveqigu.ar.events.v1` | {counts['ar']} events + asset_boundary | **PASS** |",
        f"| `docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH09.md` | Present | CH09 DC Registry | 1 entity | **PASS** |",
        "",
        "### Story 结构检查",
        "",
        "| Check | Result |",
