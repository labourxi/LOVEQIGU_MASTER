...
REPORT = ROOT / "docs/content/CH10_CONTENT_AUDIT_CREATE_REPORT.md"
CANON_PATH = ROOT / "docs/content/canon/CH10_CONTENT_CANON_V1.md"
DC_REGISTRY = ROOT / "docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH10.md"
...
CH10 = next(c for c in autopilot.load_registry() if c["code"] == "CH10")


def main() -> int:
    paths = autopilot.chapter_paths(CH10)
    audit = autopilot.validate_chapter(CH10, check_dc=True)
    ...
    if ch.get("imprint_album", {}).get("album_code") == "I":
        ok("album I")
    else:
        fail("album_code")
    ...
    if comp.get("completion_mark_relic_ref") == "relic_ch10_field_innovation_seal":
        ok("completion ref")
    else:
        fail("completion_mark_relic_ref")

    if comp.get("completion_mark") == "创新印记":
        ok("completion_mark")
    else:
        fail("completion_mark mismatch")

    if ch.get("previous_chapter") == "ch09_field_echo_future":
        ok("previous_chapter")
    else:
    ...
    for w in audit["warn"]:
        if "link:" in w:
            warn("W-001", "Low", w.replace("link: ", "CH09→CH10: "))
    ...
    seal = next(r for r in relics if r["id"] == "relic_ch10_field_innovation_seal")
    if (
    ...
    n4_ar = next(e for e in events if e["node_id"] == "n4_social_practice")
    ...
    if dc_events == ["ar_ch10_completion_v1"]:
        ok("DC only completion AR")
    else:
        fail(f"DC on wrong events: {dc_events}")

    if DC_REGISTRY.exists() and "dc_ch10_innovation_poster" in DC_REGISTRY.read_text(encoding="utf-8"):
        ok("DC registry")
    else:
        fail("DC registry missing dc_ch10_innovation_poster")

    if ch.get("next_chapter") == "TBD":
        warn("W-004", "Info", "CH10 next_chapter: TBD — CH11+ Canon 未述 · 符合暂停边界")
    ...
    lines = [
        ...
        "|------|:----------:|:------:|:------------:|:------:|",
        f"| `data/story/ch10_chapters.json` | PASS | `loveqigu.story.chapters.v1` | chapter shell + {counts['nodes']} nodes | **PASS** |",
        f"| `data/relics/ch10_relics.json` | PASS | `loveqigu.relics.v1` | {counts['relics']} relics + asset_boundary | **PASS** |",
        f"| `data/rights/ch10_rights.json` | PASS | `loveqigu.rights.v1` | {counts['rights']} rights + relic_refs_all | **PASS** |",
        f"| `data/ar/ch10_ar-events.json` | PASS | `loveqigu.ar.events.v1` | {counts['ar']} events + asset_boundary | **PASS** |",
        f"| `docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH10.md` | Present | CH10 DC Registry | 1 entity | **PASS** |",
        "",
        "### Story 结构检查",
        "",
        "| Check | Result |",
