...
REPORT = ROOT / "docs/content/CH04_CONTENT_AUDIT_CREATE_REPORT.md"
CANON_PATH = ROOT / "docs/content/CH04_CONTENT_CANON_V1.md"
CANON_PROMPT = ROOT / "prompts/CH04_CONTENT_CANON_V1.md.txt"
DC_REGISTRY = ROOT / "docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH04.md"
CH03_STORY = ROOT / "data/story/ch03_chapters.json"
...
def main() -> int:
    ...
    for layer, expected in [
        ("schema story", "loveqigu.story.chapters.v1"),
        ("schema relics", "loveqigu.relics.v1"),
        ("schema rights", "loveqigu.rights.v1"),
        ("schema ar", "loveqigu.ar.events.v1"),
    ]:
        ok(layer)
    ...
    counts = {"nodes": len(nodes), "relics": len(relics), "rights": len(rights), "ar": len(events)}
    ...
    if comp.get("completion_mark_relic_ref") == "relic_ch04_field_awakening_seal":
        ok("completion ref")
    else:
    ...
    # --- cross refs (autopilot) ---
    pass_n += len(audit["pass"])
    for w in audit["warn"]:
        if "link:" in w:
            warn("W-001", "Low", w.replace("link: ", "CH03→CH04: "))
    ...
    # memorial alignment
    seal = next(r for r in relics if r["id"] == "relic_ch04_field_awakening_seal")
    if (
    ...
    # n4 no rights on AR
    n4_ar = next(e for e in events if e["node_id"] == "n4_practice_awakening")
    ...
    # DC only on completion AR
    dc_events = [e["id"] for e in events if e.get("digital_collectible_refs")]
    if dc_events == ["ar_ch04_completion_v1"]:
        ok("DC only completion AR")
    else:
        fail(f"DC on wrong events: {dc_events}")

    if DC_REGISTRY.exists() and "dc_ch04_completion_poster" in DC_REGISTRY.read_text(encoding="utf-8"):
        ok("DC registry")
    else:
        fail("DC registry missing dc_ch04_completion_poster")

    # --- canon ---
    if ch.get("next_chapter") == "TBD":
        warn("W-004", "Info", "CH04 next_chapter: TBD — CH05+ Canon 未述 · 符合暂停边界")
    ...
    if CANON_PATH.exists() and CANON_PATH.read_text(encoding="utf-8").strip():
        ...
        warn(
            "W-005",
            "Low",
            "`docs/content/CH04_CONTENT_CANON_V1.md` 为空 · 上游 Canon 在 `prompts/CH04_CONTENT_CANON_V1.md.txt`",
        )
    else:
    ...
    dc_in_relics = any(r["id"].startswith("dc_") for r in relics)
    if dc_in_relics:
    ...
    lines = [
        ...
        "|------|:----------:|:------:|:------------:|:------:|",
        f"| `data/story/ch04_chapters.json` | PASS | `loveqigu.story.chapters.v1` | chapter shell + {counts['nodes']} nodes | **PASS** |",
        f"| `data/relics/ch04_relics.json` | PASS | `loveqigu.relics.v1` | {counts['relics']} relics + asset_boundary | **PASS** |",
        f"| `data/rights/ch04_rights.json` | PASS | `loveqigu.rights.v1` | {counts['rights']} rights + relic_refs_all | **PASS** |",
        f"| `data/ar/ch04_ar-events.json` | PASS | `loveqigu.ar.events.v1` | {counts['ar']} events + asset_boundary | **PASS** |",
        f"| `docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH04.md` | Present | CH04 DC Registry | 1 entity | **PASS** |",
        "",
        "### Story 结构检查",
        "",
        "| Check | Result |",
    ...
    lines.extend(
        [
            ...
            "",
            f"`CH04_CONTENT_AUDIT_COMPLETE = YES`",
            "",
        ]
    )
