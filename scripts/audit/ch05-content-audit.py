...
def main() -> int:
    ...
    for w in audit["warn"]:
        if "link:" in w:
            warn("W-001", "Low", w.replace("link: ", "CH04→CH05: "))
    ...
    if ch.get("next_chapter") == "TBD":
        warn("W-004", "Info", "CH05 next_chapter: TBD — CH06+ Canon 未述 · 符合暂停边界")

    if CANON_PATH.exists() and CANON_PATH.read_text(encoding="utf-8").strip():
        ok("canon md present")
    elif CANON_PROMPT.exists():
        warn("W-005", "Low", "`docs/content/CH05_CONTENT_CANON_V1.md` 为空 · 上游在 prompts（Draft）")
    else:
