...
REPORT = ROOT / "docs/content/CH08_CONTENT_AUDIT_CREATE_REPORT.md"
CANON_PATH = ROOT / "docs/content/canon/CH08_CONTENT_CANON_V1.md"
DC_REGISTRY = ROOT / "docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH08.md"
...
CH08 = next(c for c in autopilot.load_registry() if c["code"] == "CH08")


def main() -> int:
    paths = autopilot.chapter_paths(CH08)
    audit = autopilot.validate_chapter(CH08, check_dc=True)
    ...
    if comp.get("completion_mark_relic_ref") == "relic_ch08_field_legacy_seal":
        ok("completion ref")
    else:
        fail("completion_mark_relic_ref")

    if comp.get("completion_mark") == "传承印记":
        ok("completion_mark")
    else:
        fail("completion_mark mismatch")

    if ch.get("previous_chapter") == "ch07_field_echo":
        ok("previous_chapter")
    else:
    ...
    for w in audit["warn"]:
        if "link:" in w:
            warn("W-001", "Low", w.replace("link: ", "CH07→CH08: "))
    ...
    seal = next(r for r in relics if r["id"] == "relic_ch08_field_legacy_seal")
    if (
    ...
    n4_ar = next(e for e in events if e["node_id"] == "n4_collective_practice")
    ...
    if dc_events == ["ar_ch08_completion_v1"]:
        ok("DC only completion AR")
    else:
        fail(f"DC on wrong events: {dc_events}")

    if DC_REGISTRY.exists() and "dc_ch08_legacy_poster" in DC_REGISTRY.read_text(encoding="utf-8"):
        ok("DC registry")
    else:
        fail("DC registry missing dc_ch08_legacy_poster")

    if ch.get("next_chapter") == "TBD":
        warn("W-004", "Info", "CH08 next_chapter: TBD — CH09+ Canon 未述 · 符合暂停边界")
    ...
    lines = [
        ...
        "|------|:----------:|:------:|:------------:|:------:|",
        f"| `data/story/ch08_chapters.json` | PASS | `loveqigu.story.chapters.v1` | chapter shell + {counts['nodes']} nodes | **PASS** |",
        f"| `data/relics/ch08_relics.json` | PASS | `loveqigu.relics.v1` | {counts['relics']} relics + asset_boundary | **PASS** |",
        f"| `data/rights/ch08_rights.json` | PASS | `loveqigu.rights.v1` | {counts['rights']} rights + relic_refs_all | **PASS** |",
        f"| `data/ar/ch08_ar-events.json` | PASS | `loveqigu.ar.events.v1` | {counts['ar']} events + asset_boundary | **PASS** |",
        f"| `docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH08.md` | Present | CH08 DC Registry | 1 entity | **PASS** |",
        "",
        "### Story 结构检查",
        "",
        "| Check | Result |",
