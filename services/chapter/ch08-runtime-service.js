/**
 * Root Node runtime loader for CH08 L2 data.
 */
...
const story = readJson('data/story/ch08_chapters.json');
const relics = readJson('data/relics/ch08_relics.json');
const rights = readJson('data/rights/ch08_rights.json');
const ar = readJson('data/ar/ch08_ar-events.json');
const dcRegistryPath = path.join(ROOT, 'docs/content/DIGITAL_COLLECTIBLE_REGISTRY_CH08.md');
...
function validateCrossRefs() {
  ...
  const completion = getArEvents().find((e) => e.id === 'ar_ch08_completion_v1');
