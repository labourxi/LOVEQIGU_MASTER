...
Page({
  ...
  runPrimitiveSelectorDiagnostics(delayMs) {
    ...
    query.exec(() => {
      ...
      const scenicStorageResult = {
        status: nextState.primitiveSampleStatus === 'PRIMITIVE_RENDER_READY' ? 'READY' : 'BLOCKED',
        objectVisible: nextState.primitiveObjectVisible,
        blockReason: nextState.primitiveBlockReason || '',
        checkedAt: nextState.primitiveCheckedAt || nowIso(),
        source: 'xr_scenic_point_render_exact_baseline_clone'
      };

      try {
        wx.setStorageSync('XR_SCENIC_POINT_RENDER_RESULT_V1', scenicStorageResult);
      } catch (err) {
        console.warn('[XR_SCENIC_POINT_SELECTOR_ALIGNMENT_WITH_PRIMITIVE_SAMPLE_FIX_V1] SCENIC_STORAGE_WRITE_FAIL', err);
      }

      if (nextState.primitiveSampleStatus === 'PRIMITIVE_RENDER_READY') {
        console.log('[XR_SCENIC_POINT_SELECTOR_ALIGNMENT_WITH_PRIMITIVE_SAMPLE_FIX_V1] PRIMITIVE_RENDER_READY_SYNCED');
        console.log('[XR_SCENIC_POINT_SELECTOR_ALIGNMENT_WITH_PRIMITIVE_SAMPLE_FIX_V1] BLOCK_REASON_CLEARED');
      }
    });
  }
});
